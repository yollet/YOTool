...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    for (UICollectionViewLayoutAttributes *attr in atts) {
        CGFloat centerSpace = fabs(attr.center.x - self.collectionView.contentOffset.x - self.collectionView.myTWidth / 2.0); // 当前距离中心位置
        CGFloat maxSmallSc = itemMinHeight / itemMaxHeight; // 最大缩小比例
        CGFloat nextCenterSpace = self.collectionView.myTWidth / 2.0 + itemWidth / 2.0 + itemSpace; // 最小缩放所偏移的量
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity
{
    ...
    CGRect rect = CGRectMake(thisPoint.x, 0, self.collectionView.myTWidth, self.collectionView.myTHeight);
    ...
    for (UICollectionViewLayoutAttributes *attr in atts) {
        CGFloat space = attr.center.x - thisPoint.x - self.collectionView.myTWidth / 2.0; // 当前距离中心位置
